...
class Solution:
    def candy(self, _A):
        ...
        # Traverse from left to right and give candies based on increasing ratings
        for i in range(1, n):
            if self.A[i] > self.A[i - 1]:
                candies[i] = candies[i - 1] + 1

        # Traverse from right to left and adjust candies to satisfy both conditions
        for i in range(n - 2, -1, -1):
            if self.A[i] > self.A[i + 1]:
                candies[i] = max(candies[i], candies[i + 1] + 1)

        # Return the total number of candies given
        print(candies)
        return sum(candies)
...
def main():
    # D = [1, 3, 2, 1, 2, 1, 2]
    # D = [1, 3, 2, 1, 2, 1, 2]
    # A = [ -255, 369, 319, 77, 128, -202, -147, 282, -26, -489, -443, -401, 385, 465, -134, 126, 304, 179, 16, 112, 473, -467, 279, -233, 66, 76, 408, 148]
