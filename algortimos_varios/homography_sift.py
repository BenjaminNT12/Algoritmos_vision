...
def homography():
    ...
    # cv.waitKey(0)
    sift = cv.xfeatures2d.SIFT_create(nfeatures = 1000) # crea un obejto de tipo sift para hacer la localizacion de los puntos de interes
    kp_image, desc_image = sift.detectAndCompute(crop_img_gray, None) # encuentra los puntos de interes y los procesa para propocionar las coordenadas de cada uno de ellos
    ...
    flann = cv.FlannBasedMatcher(index_params, searh_params) # Fast Library for Approximate Nearest Neighbors
    ...
    while True:
        ...
        kp_grayframe, desc_grayframe = sift.detectAndCompute(grayframe, None) # detectamos los puntos de interes y los procesamos
        keygrayframe = cv.drawKeypoints(grayframe, kp_grayframe, grayframe) # dibujamos los puntos de interes

        cv.imshow("keygrayframe", keygrayframe) # mostramos el dibujo de los puntos de interes
        ...
        for m, n in matches: #
            if m.distance < 0.7 * n.distance: # distancia entre los puntos de interes, entre mas cercanos sean mejor va a hacer la coincidencia
